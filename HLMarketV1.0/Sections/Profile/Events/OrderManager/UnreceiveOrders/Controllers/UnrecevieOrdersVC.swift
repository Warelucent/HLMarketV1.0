...
private let kUnRecevieOrdersVCTableViewCellID = "kUnRecevieOrdersVCTableViewCellID"

class UnrecevieOrdersVC: BaseViewController {
    
    var dataSource:[OrderSheetServerModel]? = []
    lazy var unrecevieTableView:UITableView = {[weak self] in
        ...
        tableView.register(UnpostOrderTableCell.self, forCellReuseIdentifier: kUnRecevieOrdersVCTableViewCellID)
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(unrecevieTableView)
            ...
            AlamofireNetWork.required(urlString: "/Simple_online/Select_order", method: .post, parameters: ["UserNo":userNo, "fage":"0", "Number_of_pages":"1"], success: { (results) in
                ...
                if json["resultStatus"] == "1" {
                    ...
                    DispatchQueue.main.async(execute: {
                        self.unrecevieTableView.reloadData()
                    })
                }
...
extension UnrecevieOrdersVC:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:UnpostOrderTableCell = tableView.dequeueReusableCell(withIdentifier: kUnRecevieOrdersVCTableViewCellID, for: indexPath) as! UnpostOrderTableCell
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        let checkBtn = createBtn(title: "查看物流", bgColor: UIColor.white, borderColor: UIColor.init(gray: 232), titleColor: UIColor.black)
        
        let confirmBtn = createBtn(title: "确认收货", bgColor: UIColor.red, borderColor: UIColor.init(gray: 232), titleColor: UIColor.white)
        ...
        checkBtn.snp.makeConstraints { (make) in
            make.width.top.bottom.equalTo(confirmBtn)
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section != 0{
            let view  = UIView.init()
            view.backgroundColor = UIColor.init(gray: 232)
            return view
        } else
        {
            return nil

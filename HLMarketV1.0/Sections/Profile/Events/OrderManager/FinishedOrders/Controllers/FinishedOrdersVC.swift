...
private let kFinishedOrdersVCTableViewCellID = "kFinishedOrdersVCTableViewCellID"

class FinishedOrdersVC: BaseViewController {

    var dataSource:[OrderSheetServerModel]? = []
    
    lazy var finishedTableView:UITableView = {[weak self] in
        ...
        tableView.register(UnpostOrderTableCell.self, forCellReuseIdentifier: kFinishedOrdersVCTableViewCellID)
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        view.addSubview(finishedTableView)
        
        if let userNo = UserAuthManager.sharedManager.getUserModel()?.UserNo {
            AlamofireNetWork.required(urlString: "/Simple_online/Select_order", method: .post, parameters: ["UserNo":userNo, "fage":"3", "Number_of_pages":"1"], success: { (results) in
                print(results)
...
extension FinishedOrdersVC:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:UnpostOrderTableCell = tableView.dequeueReusableCell(withIdentifier: kFinishedOrdersVCTableViewCellID, for: indexPath) as! UnpostOrderTableCell
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        let deleteBtn = createBtn(title: "删除订单", bgColor: UIColor.white, borderColor: UIColor.init(gray: 232), titleColor: UIColor.black)

...
private let kUnpayOrderVCTableViewCellID = "kUnpayOrderVCTableViewCellID"
class UnpayOrdersVC: BaseViewController {
    
    var dataSource:[OrderSheetServerModel]? = []
    
    lazy var unpayTableView:UITableView = {[weak self] in
        ...
        tableView.register(UnpayOrderTableCell.self, forCellReuseIdentifier: kUnpayOrderVCTableViewCellID)
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(unpayTableView)
           ...
           let json = JSON(results)
            ...
            if json["resultStatus"] == "1" {
                ...
                DispatchQueue.main.async(execute: {
                    self.unpayTableView.reloadData()
                })
            }
...
extension UnpayOrdersVC:UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell:UnpayOrderTableCell = tableView.dequeueReusableCell(withIdentifier: kUnpayOrderVCTableViewCellID, for: indexPath) as! UnpayOrderTableCell
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        let cancelBtn = createBtn(title: "取消订单", bgColor: UIColor.white, borderColor: UIColor.init(gray: 232), titleColor: UIColor.black)
        
        let payBtn = createBtn(title: "付款", bgColor: UIColor.red, borderColor: UIColor.init(gray: 232), titleColor: UIColor.white)
        ...
        cancelBtn.snp.makeConstraints { (make) in
            make.width.top.bottom.equalTo(payBtn)
            make.right.equalTo(payBtn.snp.left).offset(-15)
        }
